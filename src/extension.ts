	...
	if (!apiKey) {
		...
		if (apiKey) {
			...
			const action = await vscode.window.showErrorMessage(
				'Gemini API key is required to use this extension.',
				'Set API Key',
				'Get API Key'
			);
...
async function getCodeOrPrompt(editor: vscode.TextEditor, promptMessage: string): Promise<string | undefined> {
...
async function showWelcomeView(context: vscode.ExtensionContext) {
...
function updateStatusBarItem(context: vscode.ExtensionContext, isConfigured: boolean) {
	if (!statusBarItem) {
...
async function displayResponse(response: string, editor?: vscode.TextEditor) {
	const panel = vscode.window.createWebviewPanel(
		'aiCodeAssistResponse',
		'AI Code Assistant',
...
async function generateDocumentation(apiKey: string, code: string, language: string): Promise<string> {
...
async function performCodeReview(apiKey: string, code: string, language: string): Promise<string> {
...
async function generateTestCases(apiKey: string, code: string, language: string): Promise<string> {
...
async function translateCode(apiKey: string, code: string, sourceLanguage: string, targetLanguage: string): Promise<string> {
...
async function analyzeComplexity(apiKey: string, code: string, language: string): Promise<string> {
...
async function fixErrors(apiKey: string, code: string, language: string): Promise<string> {
...
async function applyErrorFix(document: vscode.TextDocument, edit: vscode.WorkspaceEdit, fixedCode: string) {
...
function debounce<T extends (...args: any[]) => any>(
	...
	let previousPromise: Promise<ReturnType<T>> | null = null;
...
export function activate(context: vscode.ExtensionContext) {
	...
	let toggleSuggestionsDisposable = vscode.commands.registerCommand('gemini.enableSuggestions', () => {
		const config = vscode.workspace.getConfiguration('gemini');
		const currentValue = config.get<boolean>('suggestions.enabled', true);
		config.update('suggestions.enabled', !currentValue, vscode.ConfigurationTarget.Global).then(() => {
			vscode.window.showInformationMessage(
				`Real-time suggestions ${!currentValue ? 'enabled' : 'disabled'}`
			);
		});
	});
	...
	const triggerChars = config.get<string[]>('suggestions.triggerCharacters') || ['.', ' ', '(', ','];
	const debounceTime = config.get<number>('suggestions.debounceTime') || 500;
	...
	let createRefactorDisposable = vscode.commands.registerCommand('gemini.createRefactor', async () => {
		...
		const codeOrPrompt = await getCodeOrPrompt(
			editor,
			`Describe what ${language} code to create or how to refactor the selected code:`
		);
		if (!codeOrPrompt) return;
		...
		try {
			...
			const response = await withProgress('Generating code...', () => 
	...
	let askExplainDisposable = vscode.commands.registerCommand('gemini.askExplain', async () => {
		...
		const codeOrPrompt = await getCodeOrPrompt(
			editor,
			'Ask a question about the code or enter code to explain:'
		);
		if (!codeOrPrompt) return;
	...
	let fixDisposable = vscode.commands.registerCommand('gemini.fix', async () => {
		...
		try {
			...
			const response = await withProgress('Fixing code...', () =>
				callGemini(apiKey, prompt)
			);
			await displayResponse(response, editor);
		} catch (error) {
	...
	let debugDisposable = vscode.commands.registerCommand('gemini.debug', async () => {
		...
		try {
			...
			const response = await withProgress('Analyzing code for debugging...', () =>
				callGemini(apiKey, prompt)
			);
			await displayResponse(response, editor);
		} catch (error) {
	...
	context.subscriptions.push(
		welcomeDisposable,
		toggleSuggestionsDisposable,
		suggestionDisposable,
		createRefactorDisposable,
		askExplainDisposable,
		fixDisposable,
		debugDisposable,
		suggestMoreDisposable,
		generateDocDisposable,
		codeReviewDisposable,
		generateTestsDisposable,
		translateCodeDisposable,
		analyzeComplexityDisposable,
		setApiKeyDisposable
	);
